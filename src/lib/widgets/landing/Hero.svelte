<script>
	const panels = [
		{ title: 'Styles', img: 'styles.jpg', icon: '', link: '#styles' },
		{ title: 'Artists', img: 'artist.jpg', icon: '', link: '#artists' },
		{ title: 'Pieces', img: 'pieces.jpg', icon: '', link: 'pieces' },
		{ title: 'Slang', img: 'slang.jpg', icon: '', link: 'slang' }
	];
	import { links } from '$sharedData';
	import link from 'daisyui/components/link';
</script>

<!-- lg:min-h-screen xl:min-h-screen md:min-h-screen -->
<section class="min-h-screen w-full pt-[22rem] lg:pt-20">
	<!-- <nav class="w-full">
		<ul>
			<li>
				<a href="/" class="link">
					<img src="./images/logo.svg" class="" alt="Logo of boardgame: Graffiti Chronicles" />
				</a>
			</li>
		</ul>
			...
			<article class="w-full">
				<ul class="panels">
					{#each panels as panel}
						<li class="panel group max-w-md lg:w-full">
							<!-- #{panel.link} -->
							<a href={panel.link}>
								<span
									style="z-index: 90;"
									class="font-graffity text-white group-hover:text-yellow-500"
								>
									{panel.title}
...
<style>
	...
	nav {
		...
		box-shadow:
			0 0.8rem 2.4rem var(--shadow-color),
			0 1.6rem 5.6rem var(--shadow-color),
			0 2.4rem 8rem var(--shadow-color);
		display: flex;
	...
	nav ul:nth-of-type(2) li:last-of-type {
		margin-left: auto;
	}
	...
	@media (max-width: 560px) {
		...
		.geo-square img {
			-webkit-animation:
				scale-in-ver-center var(--duration) cubic-bezier(0.25, 0.46, 0.45, 0.94) forwards,
		...
		.geo-square {
			align-self: center;
			-webkit-animation:
				scale-in-hor-center var(--duration) cubic-bezier(0.25, 0.46, 0.45, 0.94) forwards,
	...
	.panel:hover {
		box-shadow:
			rgba(255, 209, 98, 0.25) 0px 30px 60px -12px inset,
			rgba(0, 0, 0, 0.3) 0px 18px 36px -18px inset;
	}
	.panel:hover span {
		color: var(--yellow);
